...
foobar(command)       ; foobar2000 控制中枢
{
    ...
    program="D:\ProgramFiles\foobar\dreamix_v3.4\foobar2000.exe"
    TrayTip,%ahkTitle%,Foobar command: %command%,3,1
    Run,%program% /%command%
...
{
    cycling=0
    TrayTip,%ahkTitle%,%myTitle%:`nCharging stopped!,3,2
    SetTimer,AutoInterval,Off
}
Else
{
    cycling=1
    TrayTip,%ahkTitle%,%myTitle%:`nCharging started!,3,2
    SetTimer,AutoInterval,-0
}
return
...
IfWinExist,^foobar2000 ahk_class {E7076D1C-A7BF-4f39-B771-BCBE88F2A2A8}
{
    ...
    if ErrorLevel {
        ...
        TrayTip,%ahkTitle%,Foobar starts to play failed!,3,3
        cycling:=0
        return
    }
}
...
IfWinExist,^foobar2000 ahk_class {E7076D1C-A7BF-4f39-B771-BCBE88F2A2A8}
{
    ...
    if ErrorLevel
        ...
        TrayTip,%ahkTitle%,next: after %rand% seconds.,3,1
        Sleep,% rand*1000
        if cycling = 0
            return
            foobar("play")
            ...
            if ErrorLevel {
                ;MsgBox,0,Error!,Foobar start playing failed!,2
                TrayTip,%ahkTitle%,Foobar start to play failed!,3,3
                cycling:=0
                return
            }
}
else IfWinExist,.* - foobar2000 ahk_class {E7076D1C-A7BF-4f39-B771-BCBE88F2A2A8}
{
    WinWait,.* - foobar2000 ahk_class {E7076D1C-A7BF-4f39-B771-BCBE88F2A2A8},,5
    if ErrorLevel
        goto CycleFoobar
...
else
{
    ...
    if ErrorLevel {
        ...
        TrayTip,%ahkTitle%,Foobar launches failed!,3,3
        cycling:=0
        return
    }
}
